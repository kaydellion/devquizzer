...
$sql = "SELECT * FROM " . $siteprefix . "languages WHERE s='$item'";
$sql2 = mysqli_query($con, $sql);
while ($row = mysqli_fetch_array($sql2)){ $categoryname=$row['title']; }

$query = "SELECT c.*, l.title AS category FROM ".$siteprefix."courses c LEFT JOIN ".$siteprefix."languages l ON c.language=l.s WHERE c.language='$item' AND c.status='publish'";
...
<div class="row p-5">
<h3 class="text-bold"><?php echo $categoryname; ?> Courses</h3>
<?php 
...
<?php }}else { 
echo "<div class='alert alert-warning' role='alert'>No courses found for this category. <a href='courses.php' class='alert-link'>Find courses</a></div>";
}
?>
